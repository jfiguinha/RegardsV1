...
#ifndef __CWMPHOST__H__
#define __CWMPHOST__H__
...
public:
    ...
    BEGIN_MSG_MAP(CWMPHost)
		...
		MESSAGE_HANDLER(WM_KEYDOWN, OnCommand)
	...
	LRESULT OnPlayFile(const char * filename, int iSize);
	LRESULT OnClose();
	LRESULT OnWMPPlayer2StretchToFit();
	LRESULT OnWMPPlayerFullScreen();
	LRESULT OnGetFullScreenState();

	LRESULT GetCurrentPosition(double &dValue);
	LRESULT SetCurrentPosition(const double &dValue);

	LRESULT CALLBACK CreateBookmarkFile();
	LRESULT CALLBACK ReadBookmarkFile();
