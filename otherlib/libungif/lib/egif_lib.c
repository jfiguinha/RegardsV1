...
GifFileType *EGifOpenFileName(char *FileName, int TestExistance)
{
    ...
    if (TestExistance)
	FileHandle = open(FileName,O_WRONLY | O_CREAT | O_EXCL | O_BINARY, S_IREAD | S_IWRITE);
    else
	FileHandle = open(FileName,O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, S_IREAD | S_IWRITE);
...
#if defined(__MSDOS__) || defined(__GNUC__) || defined(WIN32)
