// Redirector.cpp: implementation of the CRedirector class.
//
//////////////////////////////////////////////////////////////////////
...
BOOL CRedirector::Open(LPCTSTR pszCmdLine)
{
	...
	__finally
	{
		if (!bOK)
		{
			...
			char szMsg[40];
			StringCchPrintf(szMsg,40, "Redirect console error: %x\r\n", dwOsErr);
			WriteStdError(szMsg);
			DestroyHandle(hStdoutReadTmp);
			DestroyHandle(hStdoutWrite);
			DestroyHandle(hStderrWrite);
...
BOOL CRedirector::Printf(LPCTSTR pszFormat, ...)
{
	...
	StringCchPrintf(strInput,2048, argList,pszFormat);
	//strInput.FormatV(pszFormat, argList);
	va_end(argList);

	DWORD dwWritten;
	return WriteFile(m_hStdinWrite, (LPCTSTR)strInput,strlen(strInput), &dwWritten, NULL);
}
//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
...
DWORD WINAPI CRedirector::OutputThread(LPVOID lpvThreadParam)
{
	...
	pRedir->SignalApplicationEnding();
	// close handles
	pRedir->Close();
	return nRet;
}

void CRedirector::SignalApplicationEnding()
{
