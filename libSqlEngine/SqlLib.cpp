...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
bool CSqlLib::OpenConnection(const string &databasePath, const bool &readonly, const bool &load_inmemory)
{
    ...
    // and convert string to UTF-16 (=UChar*):
    ...
    */
    ...
    //const char * fichier = CConvertUtility::ConvertFromstring(sqLiteDBPath);
    int rc = 0;
	...
	if(load_inmemory)
	{
		...
		if (PathFileExists(sqLiteDBPath.c_str()))
		{
			rc = LoadOrSaveDb(pCon, sqLiteDBPath.c_str(), 0);
	...
	{
		if(readonly)
			rc = sqlite3_open_v2(sqLiteDBPath.c_str(), &(pCon), SQLITE_OPEN_READONLY, NULL);
		else
...
void CSqlLib::CloseConnection()
{
	...
	{
		if (!readonly)
			LoadOrSaveDb(pCon, sqLiteDBPath.c_str(), 1);
		sqlite3_close(pCon);
		pCon = nullptr;
	}
}
...
bool CSqlLib::ExecuteSQLBlobInsert(const string &query, const int &numCol , const void * zBlob, const int &nBlob, CSqlResult * sqlResult)
{
	if (!isConnected())
		return 0;
...
bool CSqlLib::ExecuteSQLSelect(const string &query, CSqlResult * sqlResult)
{
	if (!isConnected())
		return 0;
...
int CSqlLib::ExecuteSQLWithNoResult(const string &query)
{
	int totalChange = -1;
	...
	//sync.lock();
    //const char * sqlquery = CConvertUtility::ConvertFromstring(query);
    //printf("Sql Query : %s \n",sqlquery);
