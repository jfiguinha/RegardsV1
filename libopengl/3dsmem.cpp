   ...
   chunk3ds_t	chunk;
	...
	do
	{
		...
		case 0x4100: // TRIANGLE_OBJECT
			StringCchCopy(output->meshes[output->meshCount].name,32, pData + 6);
...
void ParseMaterialGroup(char* pData, group3ds_t* group)
{
   ...
   pName = pData;
	...
	if (group->tris)
	{
		...
		group->mat = 0;
		StringCchCopy(group->name,32, pName);
	...
	do
	{
		...
		switch (chunk.id)
		{
			case 0xa000: // MAT_NAME
				StringCchCopy(material->name,32, pData + lPos + 6);
				break;
	...
	do
	{
		...
		switch (chunk.id)
		{
			case 0xa300: // MAT_MAP_NAME
				StringCchCopy(material->texture,32, pData + lPos + 6);
				break;
