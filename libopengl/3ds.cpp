...
static void CalculateMeshNormals(mesh3ds_t* mesh)
{
   ...
   //======================//
	...
	//:://MessageBox(NULL,"allocate the normals","infos",0);
   ...
   //:://MessageBox(NULL,"index the vertices based on position alone","infos",0);
   ...
   //:://MessageBox(NULL,"allocate a buffer for the flat normals","infos",0);
      ...
      //==============================================//
		...
		for (int i = 0; i < mesh->triCount; i++)
		{
         ...
         for (int j = 0; j < 3; j++)
         {
			...
			for (int k = 1; k <= triRefs[vert][0]; k++)
			{
   ...
   for (int i = 0; i < mesh->triCount; i++)
   {
	...
	for (int i = 0; i < mesh->triCount; i++)
	{
      for (int j = 0; j < 3; j++)
      {
         ...
         tanCount = 0;
			for (int k = 1; k <= tanRefs[mesh->tris[i][j]][0]; k++)
         {
   ...
   for (int i = 0; i < mesh->triCount; i++)
   {
           ...
           (mesh->verts[mesh->tris[i][1]][2] == mesh->verts[mesh->tris[i][2]][2])))
      {
         ...
         for (int j = 0; j < mesh->groupCount; j++)
         {
            ...
            if (found != NULL)
            {
               int k = ((int)found - (int)mesh->groups[j].tris) / sizeof(long);
               ...
               for (int l = k; l < mesh->groups[j].size; l++)
               {
...
void ReadEditObject(chunk3ds_t* chunk, data3ds_t* output)
{
	...
	case 0x4100: // TRIANGLE_OBJECT
      StringCchCopy(output->meshes[output->meshCount].name,32, g_szString);
