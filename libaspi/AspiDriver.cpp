...
BOOL CAspiDriver::IsMMC(LPSTR lpszInfo, int iSize)
{ 
	BYTE pbtBuffer[0xFF];
...
//	SCSICDMODEPAGE2A* pPage=(SCSICDMODEPAGE2A*)(pbtBuffer+sizeof(SCISMODEHEADER)+pBlockHeader->nBlockLen);
    ...
    if((pbtBuffer[4]&0x3F)==0x2A)
	{
		...
		if(pbtBuffer[5]>=4)
		{
			if(pbtBuffer[9]&0x1)
			{
				StringCchCopy(lpszInfo,iSize,"Drive is MMC style");
				return TRUE;
			}
		}
		else
		{
			StringCchCopy(lpszInfo,iSize,"MMC drive, but reports CDDA incapable");
			return TRUE;
		}
    }
	StringCchCopy(lpszInfo,iSize,"Drive does not support MMC");
	return FALSE;
}

void CAspiDriver::GetDeviceString(string & Vendor, string & Product, string & Revision, string & BusAddress)
{
	SRB_ExecSCSICmd cmd;
	BYTE Buffer[100];
	char m_cValue[100];
	...
	StringCchPrintf(m_cValue,100,"%d:%d:%d",m_Ha,m_Tgt,m_Lun);
