...
void CSptiDriver::Initialize()
{
	...
	{
		char m_cValue[20];
		StringCchPrintf(m_cValue,20,"%c:\\",i+'A');
		if(GetDriveType(m_cValue) == DRIVE_CDROM){
...
void CSptiDriver::ExecuteCommand(SRB_ExecSCSICmd &cmd)
{
	...
	{
		m_Buffer = (BYTE *)malloc(cmd.SRB_BufLen);
		m_SptiCmd.sptd.DataBuffer = m_Buffer;
		if(cmd.SRB_Flags & SRB_DIR_OUT)
		{
			memcpy(m_Buffer,cmd.SRB_BufPointer,cmd.SRB_BufLen);
		}
	}
	...
	if(((DWORD)(cmd.SRB_BufPointer) & 0x0f) != 0){
		if(cmd.SRB_Flags & SRB_DIR_IN){
			memcpy(cmd.SRB_BufPointer,m_Buffer,cmd.SRB_BufLen);
		}
	}
...
void CSptiDriver::SetDevice(int DeviceNo)
{
	...
	StringCchPrintf(cs,256,"\\\\.\\%c:",m_Address[m_CurrentDevice]);
	m_hDevice = CreateFile(cs,GENERIC_READ | GENERIC_WRITE,FILE_SHARE_READ,NULL,OPEN_EXISTING,0,NULL);
}

void CSptiDriver::GetDeviceString(string & Vendor, string & Product, string & Revision, string & BusAddress)
{
	//   OK
	SRB_ExecSCSICmd cmd;
	BYTE Buffer[100];
	...
	StringCchPrintf(m_cBuffer,100,"%c:",m_Address[m_CurrentDevice]);
	BusAddress = m_cBuffer;
}
