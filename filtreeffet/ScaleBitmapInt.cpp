#include "StdAfx.h"
#include "ScaleBitmapInt.h"


CScaleBitmapInt::CScaleBitmapInt(void)
{
}
...
HBITMAP CScaleBitmapInt::ScaleBitmap(HBITMAP hBmp,
                       WORD wNewWidth,
                       WORD wNewHeight)
{
 BITMAP bmp;
...
BITMAPINFO * CScaleBitmapInt::PrepareRGBBitmapInfo(WORD wWidth, WORD wHeight)
{
...
int * CScaleBitmapInt::CreateCoeff(int nLen, int nNewLen, BOOL bShrink)
{
 int nSum = 0, nSum2;
...
void CScaleBitmapInt::ShrinkData(BYTE *pInBuff,
                   WORD wWidth,
                   WORD wHeight,
                   BYTE *pOutBuff,
                   WORD wNewWidth,
                   WORD wNewHeight)
{
 ...
 int   *pRowCoeff = CreateCoeff(wWidth,
                                   wNewWidth,
                                   TRUE);
 int   *pColCoeff = CreateCoeff(wHeight,
                                   wNewHeight,
                                   TRUE);
...
void CScaleBitmapInt::EnlargeData(BYTE *pInBuff,
                    WORD wWidth,
                    WORD wHeight,
                    BYTE *pOutBuff,
                    WORD wNewWidth,
                    WORD wNewHeight)
{
 ...
 int   x, y, i;
 BOOL  bCrossRow, bCrossCol;
 int   *pRowCoeff = CreateCoeff(wNewWidth,
                                   wWidth,
                                   FALSE);
 int   *pColCoeff = CreateCoeff(wNewHeight,
                                   wHeight,
                                   FALSE);
 int   *pXCoeff, *pYCoeff = pColCoeff;
