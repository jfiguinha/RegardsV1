...
double CFast2PassScale::Filter(const double &dVal)
{
    double m_dLocalVal = fabs(dVal); 
    return (m_dLocalVal < m_dWidth ? m_dWidth - m_dLocalVal : 0.0); 
}

CFast2PassScale::LineContribType * CFast2PassScale::AllocContributions (int uLineLength, int uWindowSize)
{
...
CFast2PassScale::LineContribType * CFast2PassScale::CalcContributions (int uLineSize, int uSrcSize, float dScale)
{ 

	float dWidth;
    float dFScale = 1.0f;
    float dFilterWidth = m_dWidth;//1.0f;//CurFilter.GetWidth();
    ...
    {   // Scan through line of contributions
        ...
        float dTotalWeight = 0.0f;  // Zero sum of weights
		float dTotalWeight2 = 0.0f;  // Zero sum of weights
		float dVal;
		float filtered;
		int iSrc;
		...
		{   
			...
			filtered = (float)(dVal < 1.0f ? 1.0f - dVal : 0.0f); 
