...
BOOL GenericHTTPClient::Request(LPCTSTR szURL, int iUrlSize, int nMethod, LPCTSTR szAgent){
	...
	ParseURL(szURL, iUrlSize, szProtocol, __SIZE_BUFFER, szAddress, __SIZE_BUFFER, dwPort, szURI,__SIZE_BUFFER);
...
BOOL GenericHTTPClient::RequestPostMultiPartsFormData(LPCTSTR szURI){
	...
	_stprintf_s( szContentLength, 1024, "Content-Length: %d\r\n", dwPostBufferLength);
...
VOID GenericHTTPClient::AddPostArguments(LPCTSTR szName, LPCTSTR szValue, BOOL bBinary){
	...
	_tcscpy_s(arg.szName,256, szName);
	_tcscpy_s(arg.szValue,1024, szValue);
...
VOID GenericHTTPClient::AddPostArguments(LPCTSTR szName, DWORD nValue){
	...
	_tcscpy_s(arg.szName,256, szName);
	sprintf_s(arg.szValue,1024, "%d", nValue);
	arg.dwType = GenericHTTPClient::TypeNormal;
...
DWORD GenericHTTPClient::GetPostArguments(LPTSTR szArguments, DWORD dwLength){
	...
	for(itArg=_vArguments.begin(); itArg<_vArguments.end(); )
	{
		...
		_tcscat_s(szArguments, dwLength, "=");
...
DWORD GenericHTTPClient::AllocMultiPartsFormData(PBYTE &pInBuffer, LPCTSTR szBoundary){
	...
	for(itArgv=_vArguments.begin(); itArgv<_vArguments.end(); ++itArgv){
		...
		case	GenericHTTPClient::TypeBinary:
			...
			_stprintf_s( (TCHAR*)pBuffer,__SIZE_HTTP_HEAD_LINE*3+dwSize+1,
							"--%s\r\n"
...
LPCTSTR GenericHTTPClient::GetContentType(LPCTSTR szName){
	...
	if(RegOpenKeyEx(HKEY_CLASSES_ROOT, szWord, 0, KEY_QUERY_VALUE, &hKEY)==ERROR_SUCCESS){
		if(RegQueryValueEx(hKEY, TEXT("Content Type"), NULL, NULL, (LPBYTE)szReturn, (unsigned long*)&dwLen)!=ERROR_SUCCESS)
			_tcsncpy_s(szReturn,1024, "application/octet-stream", _tcslen("application/octet-stream"));
		RegCloseKey(hKEY);
	}else{
		_tcsncpy_s(szReturn,1024, "application/octet-stream", _tcslen("application/octet-stream"));
	}
...
VOID GenericHTTPClient::ParseURL(LPCTSTR szURL, int iUrlSize, LPTSTR szProtocol, int iProtocolSize, LPTSTR szAddress, int iAddressSize, DWORD &dwPort, LPTSTR szURI, int iURISize){
	...
	if(!_tcsncmp((szURL+dwPosition+1), "/", 1)){	// is PROTOCOL
		if(szProtocol){
			_tcsncpy_s(szProtocol, iProtocolSize, szURL, dwPosition);
			szProtocol[dwPosition]=0;
		}
		bFlag=TRUE;
	}else{	// is HOST 
		if(szProtocol){
			_tcsncpy_s(szProtocol, iProtocolSize, "http", 4);
			szProtocol[5]=0;
		}
	}
	...
	if(bFlag){
		...
		dwPort=_ttol(sztmp);
		_tcsncpy_s(szAddress, iAddressSize, (szURL+dwStartPosition), dwFind-dwStartPosition);
	}else if(!_tcscmp(szProtocol,"https")){
		dwPort=INTERNET_DEFAULT_HTTPS_PORT;
		_tcsncpy_s(szAddress, iAddressSize, (szURL+dwStartPosition), dwPosition-dwStartPosition);
	}else {
	...
	if(dwPosition<_tcslen(szURL)){ // find URI
		_tcsncpy_s(szURI, iURISize, (szURL+dwPosition), _tcslen(szURL)-dwPosition);
	}else{
		szURI[0]=0;
	}
